...
Differential drive is a drive of two independent motors or wheels on the same axis. This means the right and the left wheel each have their own motor. Wheels on other axes don't need to be driven. If both wheels drive forward, the robot also goes forward. If both go backwards, the robot goes backwards. However, if for example the left wheel goes faster than the right one, then the robot does a curve to the right, because its left side is faster and "overtakes" the right side. The name "differential" comes from the wheels' difference in rotation speed which decides about the direction the robot goes.
